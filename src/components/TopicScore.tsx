...
import { SCORE_LABELS } from "@/types/reviews";
  ...
  return (
    <>
      <button
        ...
        title={
          saved
            ? `${SCORE_LABELS[saved.score]} — rated by ${saved.ratedBy}`
            : "Rate this part (0–3)"
        }
        className={`inline-flex items-center gap-1 rounded-md border px-2.5 py-1 text-xs font-medium shadow-sm disabled:cursor-not-allowed disabled:opacity-50 ${
          display !== null
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={`Score — ${label}`}>
        ...
        <p className="mb-4 text-sm text-slate-500">
          Rate <strong>{label}</strong> from 0 (not acceptable) to 3 (ready for sign-off).
        </p>
